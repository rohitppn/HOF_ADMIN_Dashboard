...
export function handleOwnerCommand({ text, senderPhone }) {
  ...
  switch (cmd) {
    ...
    case '/report': {
      ...
      if (sub === 'store') {
        ...
        const rows = queries.dsrRange(from, to).filter((r) => r.store_key === store.key);
